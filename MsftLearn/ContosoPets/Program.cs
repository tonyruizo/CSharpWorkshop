            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 2] == "Age: ?" && ourAnimals[i, 0] != "ID: ")
                {
                    do
                    {
                        ...
                        {
                            animalAge = readResult;
                            // If true. Will parse number string into an integer as 'petAge'
                            validEntry = int.TryParse(animalAge, out petAge);
                        }
                        // if entry is not parse into an integer, loop again.
                    } while (validEntry == false);
                ...
                if (ourAnimals[i, 4] == "Physical description: " && ourAnimals[i, 0] != "ID #: ")
                {
                    do
                    {
                        System.Console.WriteLine($"Enter a physical description for {ourAnimals[i, 0]} (size, color, gender, weight, housebroken)");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
                            animalPhysicalDescription = readResult.ToLower();
